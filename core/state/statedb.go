...
func (s *StateDB) commitAndFlush(block uint64, deleteEmptyObjects bool, noStorageWiping bool, deriveCodeFields bool) (*StateUpdate, error) {
	...
	// debank: stream the committed state mutations to the live pipeline tracer.
	// In v1.17.x the snapshot and trie-database updates that used to live here are
	// performed inside Database.Commit above; only the onCommit hook that feeds
	// github.com/Chaintable/pipeline is retained. The StateUpdate is re-encoded
	// into the slim-RLP/trimmed-RLP blob format the hook expects via EncodeMPTState.
	if s.onCommit != nil && !ret.Empty() {
		contracts := make(map[common.Hash][]byte)
		for _, code := range ret.Codes {
			contracts[code.Hash] = code.Blob
		}
		accountsBlob, accountsOrigin, storages, storagesOrigin := ret.EncodeMPTState()
		destructs := make(map[common.Hash]struct{})
		accounts := make(map[common.Hash][]byte)
		for addr, v := range accountsBlob {
			if v == nil {
